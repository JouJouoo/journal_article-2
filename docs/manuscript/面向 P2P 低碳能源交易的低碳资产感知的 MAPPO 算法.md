...
考虑由 $N$ 个产消者组成的本地能源社区，产消者集合记为 $\mathcal{N}=\{1,2,\cdots,N\}$，调度周期被划分为 $T$ 个离散时段，时段集合记为 $\mathcal{T}=\{1,2,\cdots,T\}$。在时段 $t$，产消者 $i$ 具有本地负荷 $L_{i,t}$、光伏出力 $G_{i,t}$ 和储能荷电状态 $SOC_{i,t}$。产消者可以通过 P2P 市场与其他用户交易电能，也可以与上级电网进行补充购售电。其主要决策包括从 P2P 市场购电量 $P^{p2p,b}_{i,t}$、向 P2P 市场售电量 $P^{p2p,s}_{i,t}$、从电网购电量 $P^{g,b}_{i,t}$、向电网售电量 $P^{g,s}_{i,t}$ 以及储能充放电功率 $P^{ch}_{i,t}$、$P^{dis}_{i,t}$。
...
$$
o_{i,t}=\left[
L_{i,t},G_{i,t},\pi^{p2p}_{t},\pi^{g,b}_{t},\pi^{g,s}_{t},
SOC_{i,t},\tilde{B}_{i,t}
\right],
$$

其中，$\pi^{p2p}_{t}$ 为 P2P 交易价格，$\pi^{g,b}_{t}$ 和 $\pi^{g,s}_{t}$ 分别为电网购电价格和售电价格，$\tilde{B}_{i,t}$ 为归一化后的 LCCoins 余额。本文将低碳资产状态定义为：
...
其中，$a_{i,t}$ 包括 P2P 交易、电网购售电和储能充放电决策。训练阶段，actor 采用低碳资产分解优势和自适应裁剪系数更新：
...
$$
r^{eco}_{i,t}=
\pi^{p2p}_{t}(P^{p2p,s}_{i,t}-P^{p2p,b}_{i,t})\Delta t
+\pi^{g,s}_{t}P^{g,s}_{i,t}\Delta t
-\pi^{g,b}_{t}P^{g,b}_{i,t}\Delta t
-c^{bat}_{i}(P^{ch}_{i,t}+P^{dis}_{i,t})\Delta t,
$$

其中，$c^{bat}_{i}$ 为储能充放电退化成本系数。低碳资产存量效用采用 CRRA 函数刻画 LCCoins 余额带来的长期资产价值：
...
基于上述交易环境、LCCoins 生成机制、低碳资产状态和动态资产效用奖励函数，本文形成低碳资产优势分解 MAPPO 算法。每个 episode 中，环境首先初始化负荷、光伏、价格、储能 SOC 和 LCCoins 账户余额；随后各产消者根据包含常规运行状态和 LCCoins 余额的观测选择交易与储能动作；环境执行动作并完成 P2P 市场清算、储能状态更新和功率平衡检查；区块链共识模块根据清算结果计算低碳贡献，并在验证节点达成确认后铸造 LCCoins、更新账户余额；最后分别计算经济回报和 LCCoins 低碳资产回报，并通过优势分解、信用分配和 LCCoins 资产状态自适应 PPO 裁剪完成策略更新。
...
3. 初始化 episode 中的负荷、光伏、价格和储能状态。
4. 对每个时段 $t=1,2,\cdots,T$：
5. 每个产消者构造观测 $o_{i,t}$，其中包含负荷、光伏、价格、$SOC_{i,t}$ 和 LCCoins 余额 $\tilde{B}_{i,t}$。
6. 每个 actor 根据 $\pi_{\theta_i}(a_{i,t}|o_{i,t})$ 采样交易与储能动作。
7. 环境执行动作，完成 P2P 清算、功率平衡约束处理和 $SOC_{i,t+1}$ 更新。
