...
def _plot_learning_curves(rows: list[dict], output_dir: Path) -> list[Path]:
    ...
    for (label,), label_rows in by_label.items():
        ...
        plotted = False
        for (variant,), variant_rows in sorted(
            by_variant.items(), key=lambda item: variant_sort_key(str(item[0][0]))
        ):
            if str(variant) not in TRAINABLE_VARIANTS:
                continue
            curves = []
        ...
        if not plotted:
            plt.close(fig)
            continue
        ax.set_title(f"Training reward - {format_label(label) if label else 'default'}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create figures from LC-MAPPO experiment summary.json files."
    )
